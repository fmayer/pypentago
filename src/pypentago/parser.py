class State(object):
    ...
    def change_to_state(self):
        if self.parsed[-1]:
            self.parsed.append('')
    def get_result(self):
        return [elem.strip() for elem in self.parsed if not elem == ""]
...
class Parser(object):
    ...
    def get_state_by_char(self, char):
        ...
        for state in self.states:
            if not state.state_char == None and state.state_char == char:
                return state
        for state in self.states:
            if not state.exit_char == None and state.exit_char == char:
                return self.default_state
